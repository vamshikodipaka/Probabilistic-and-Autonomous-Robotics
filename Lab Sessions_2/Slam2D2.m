...
% Sensor Range - Observing (x,v,landmarks)
%
...
% Definition of process and initial noises
r = (3*0.5)^2;  % landmark measurement noise, change to see effect
rx = (20*0.5)^2; % position measurement noise, change to see effect
rv = (2*0.5)^2; % velocity measurement noise, change to see effect
...
for k = 1:NumberTimeStamps-2
    ...
    znot(:,k+1)= H*[x(:,k+1)' v(:,k+1)' p]'; % Measure at the correct position x(k+1) v(k+1) and p
    Ht = H; Rt = R; % Temporal measuring matrices of observed features
    landdec = []; landnotdec = []; % landmarks detected/non-detected sets
    vvt=z(1:4,k+1)-zhat(1:4,k+1);  % Innovation vector, i.e. discrepancy between measures
    for kk=5:2:5+2*NumberLandmarks-1  % Explore the detection of landmarks
        if norm(znot(kk:kk+1,k+1)) < MaximumRange   % Landmark in not-noisy range?       
            vvt=[vvt;z(kk:kk+1,k+1)-zhat(kk:kk+1,k+1)];  % Innovation vector, i.e. discrepancy between measures
        else
            landnotdec = [landnotdec; (kk-1)/2-1]; % Add non-detected landmark identifier to the set
        end
    end
    for kk=size(landnotdec,1):-1:1  % Removing rows/column of non-detected landmarks
        Ht(4+2*landnotdec(kk)-1:4+2*landnotdec(kk),:) = []; 
        Rt(4+2*landnotdec(kk)-1:4+2*landnotdec(kk),:) = []; 
        Rt(:,4+2*landnotdec(kk)-1:4+2*landnotdec(kk)) = [];
    end
    S = Ht*Pk*Ht' + Rt;
