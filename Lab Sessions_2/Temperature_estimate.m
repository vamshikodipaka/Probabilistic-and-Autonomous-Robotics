...
r = 20;  % measurement noise variance, change to see effect
pn = 0.02;  % process noise variance, change to see effect
...
for k = 1:NumberTimeStamps-1
    ...
    zhat(k+1) = ....            % Measure at the predicted state
    Pk(k+1) = ....
    
    % Observation
    N = normrnd(0,sqrt(r));     % Normal distribution with 0 mean and r standard deviation
    z(k+1) = ....               % Measure at the correct state
    vv(k+1) = ....              % Innovation vector, i.e. discrepancy between measures
...
for i = 1:NumberTimeStamps
    Uncertainty1(i) = xhat(i)+2*sqrt(Pk(i));
    Uncertainty2(i) = xhat(i)-2*sqrt(Pk(i));
end
